...
from transformer_maskgit.attention_temporal_fusion import Transformer_Temporal_Fusion, ContinuousPositionBias
...
class CTViT_Temporal_Fusion_butCLS(nn.Module):
    def __init__(
        self,
        *,
        dim,
        ...
        self.enc_temporal_transformer = Transformer_Temporal_Fusion(depth = temporal_depth, **transformer_kwargs)
    ...
    def encode(
        ...
        text = repeat(text, 'b one d -> (b h w) one d', h=h, w=w)

        tokens = self.enc_temporal_transformer(tokens, text=text, video_shape = video_shape)    # (B H W) D+1 Dim

        tokens = rearrange(tokens, '(b h w) t d -> b t h w d', b = b, h = h, w = w)

        return tokens[:, :-1, :, :, :],  tokens[:, -1:, :, :, :]    # Text as CLS token at the end
    ...
    def forward(
        ...
        tokens = self.to_patch_emb(video)   # B D H W dim
        # save height and width in  

        # encode - spatial

        tokens, text = self.encode(tokens, text)    # B D H W dim, B 1 H W dim
        ...
        *_, h, w, _ = shape
        ...
        # tokens, packed_fhw_shape = pack([tokens], 'b * d')  # B (D H W) dim
        # tokens, indices, commit_loss = self.vq(tokens, mask = None)  # B (D H W) dim
        # tokens = rearrange(tokens, 'b (t h w) d -> b t h w d', h = h, w = w)
        
        # text = rearrange(text, 'b o h w d -> b (o h w) d')
        # text, indices, commit_loss = self.vq(text, mask = None)  # B (1 H W) dim
        # text = rearrange(text, 'b (o h w) d -> b o h w d', o = 1, h=h, w=w)
...
if __name__ == '__main__':
    ...
    video = torch.zeros((2, 1, 240, 480, 480))
    image_encoder(video)