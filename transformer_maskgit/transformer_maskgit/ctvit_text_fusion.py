...
from transformer_maskgit.attention_text_fusion import Temporal_Transformer_Text_Fusion, Spatial_Transformer_Text_Fusion, ContinuousPositionBias
...
class CTViT_Text_Fusion(nn.Module):
    def __init__(
        self,
        *,
        dim,
        ...
        self.enc_spatial_transformer = Spatial_Transformer_Text_Fusion(depth = spatial_depth, **transformer_kwargs)
        self.enc_temporal_transformer = Temporal_Transformer_Text_Fusion(depth = temporal_depth, **transformer_kwargs)
        self.vq = VectorQuantize(dim = dim, codebook_size = codebook_size, use_cosine_sim = True)
        
        self.pe1d = PositionalEncoding1D(dim)
    ...
    def encode(
        ...
        tokens = rearrange(tokens, 'b t h w d -> (b t) h w d')    # bd h w dim
         
        pe = self.pe2d(tokens)  # bd h w dim

        tokens = rearrange(tokens, 'bt h w d -> bt (h w) d')    # bd hw dim
        
        pe = rearrange(pe, 'bt h w d -> bt (h w) d')    # bd hw dim
        
        device=torch.device('cuda')
        attn_bias = self.spatial_rel_pos_bias(h, w, device = device)    # 8 24*24 24*24
        attn_bias = F.pad(attn_bias, (0, 1, 0, 1))  # 8 24*24+1 24*24+1
        
        aligned_text = repeat(text, 'b one dim -> (b d) one dim', d=d)  # bd 1 dim

        spatial_tokens, CLS_tokens = self.enc_spatial_transformer(tokens, aligned_text, attn_bias = attn_bias, video_shape = video_shape)   # bd hw dim, bd 1 dim

        # encode - temporal
        
        CLS_tokens = rearrange(CLS_tokens[:, 0, :], '(b t) d -> b t d', b = b)   # b d dim
        
        pe = self.pe1d(CLS_tokens)  # b d dim

        temporal_tokens, CLS_tokens = self.enc_temporal_transformer(CLS_tokens, text, pe=pe, video_shape = video_shape)   # b d dim, b 1 dim
    ...
    def forward(
        ...
        *_, h, w, _ = shape
        ...
        tokens = self.encode(tokens, text)
...
if __name__ == '__main__':
    ...
    video = torch.zeros((4, 1, 240, 480, 480)).cuda()
    text = torch.zeros((4, 1, 512)).cuda()
    tokens = image_encoder(video, text, return_encoded_tokens=True)
    print(tokens.shape)