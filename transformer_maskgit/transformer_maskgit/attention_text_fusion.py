...
class Temporal_Transformer_Text_Fusion(nn.Module):
    def __init__(
        self,
        dim,
        *,
    ...
    def forward(
        ...
        if text.ndim == 2:
            text = text.unsqueeze(1)    # b 1 dim

        for peg, self_attn, cross_attn, ff in self.layers:
            # Temporal Transformer 不保留PE（基本等于重新训）
            x = x + pe

            x = torch.concat([x, text], dim=1)   # b n+1 dim
        ...
        return self.norm_out(x), self.norm_out(text)
    
class Spatial_Transformer_Text_Fusion(nn.Module):
    def __init__(
        self,
        dim,
        *,
