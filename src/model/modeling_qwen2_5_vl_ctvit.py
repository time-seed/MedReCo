...
from .configuration_qwen2_5_vl_ctvit import Qwen2_5_VLConfig, Qwen2_5_VLTextConfig, Qwen2_5_VLVisionConfig
from .ctvit import CTViT

logger = logging.get_logger(__name__)

# TODO：2 激活函数使用silu，这是一个细节。先跑通后再调整
...
@auto_docstring
class Qwen2_5_VLPreTrainedModel(PreTrainedModel):   # 是一个基础类
...
class CTViTForQwen(Qwen2_5_VLPreTrainedModel):
    """CTViT作为Qwen2.5-VL视觉编码器的封装类"""
    ...
    def __init__(self, vision_config):
        ...
        self.ctvit = CTViT(
            ...
            final_dim=vision_config.out_hidden_size,
            image_size=vision_config.image_size,
            patch_size=vision_config.patch_size,
            temporal_patch_size=vision_config.temporal_patch_size if hasattr(vision_config, 'temporal_patch_size') else 10,
            spatial_depth=vision_config.spatial_depth,  # 分配深度
            temporal_depth=vision_config.temporal_depth if hasattr(vision_config, 'temporal_depth') else 6,
            cls_depth=vision_config.cls_depth if hasattr(vision_config, 'cls_depth') else 2,
    ...
    def forward(self, pixel_values, **kwargs):
        # 处理视觉输入
        features = self.ctvit(pixel_values, **kwargs)
        return features
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    base_model_prefix = ""
    _checkpoint_conversion_mapping = {"^model": "language_model"}  # 加载模型时将原始的权重参数中的model修改为language_m
    ...
    def __init__(self, config):
        super().__init__(config)
        self.visual = CTViTForQwen(config.vision_config)
        ...
        # print('self.config.output_hidden_states',self.config.output_hidden_states)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if pixel_values is not None:
            # TODO：这里应该修改pixel_values的格式。保证和vit的要求一致即可！！！
            # print('pixel_values',pixel_values.shape) # 8463 1176 经过vision encoder变成，返回的尺寸是可以不一样的！！！
            image_embeds = self.get_image_features(pixel_values, image_grid_thw) # 由于不同图片的尺寸不一定一样。这里使用的其实是元组
            # print('image_embeds',len(image_embeds),image_embeds[0].shape) # 4， 529 3584 # TODO: 重点在这里，只需要匹配到尺寸一致就可以。一起修改这部分内容
            image_embeds = torch.cat(image_embeds, dim=0).to(inputs_embeds.device, inputs_embeds.dtype)
            # print('image_embeds',image_embeds.shape) # 2116 3584
            # print('inputs_embeds',inputs_embeds.shape) # 4 673 3584
            # print('input_ids',input_ids.shape)  # 4 673
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        print('qwen2.5 vl for conditional generation init config',config)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # print(f"lm_head weight dtype: {self.lm_head.weight.dtype}")
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if cache_position[0] != 0:  # 只在第一步时，才计算视觉特征
            model_inputs["pixel_values"] = None
            model_inputs["pixel_values_videos"] = None
        # print('model_inputs in prepare_inputs_for_generation',model_inputs)
        return model_inputs
