...
class TransformerCLS(nn.Module):
    ...
    def forward(
        ...
        if condition_embedding is None:
            ...
            # 得到结果就是condition的结果
            cls_token = condition_embedding.unsqueeze(1)    # 这里是变成 (B*local_B, 1, dim)
            x = torch.cat((cls_token, x), dim=1)  # 在序列的开头添加CLS token
        for peg, self_attn, cross_attn, ff in self.layers:
            if exists(peg):
                # print("[DEBUG] TransformerCLS forward PEG", x.shape, x.dtype, x.device, flush=True)
                # print("[DEBUG] video_shape:", video_shape, flush=True)
