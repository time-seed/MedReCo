...
def configure_vision_tower(model, training_args, compute_dtype, device):
    ...
    ## FIXME: 修改的部分
    merger_params = model.visual.ctvit.to_visual_latent.parameters()
    set_requires_grad(merger_params, not training_args.freeze_merger)
...
def train():
    ...
    print(f"当前进程使用的GPU: {device}")
    print(f"local_rank in train(): {local_rank}")
    use_liger = training_args.use_liger
    # if "Qwen2.5" in model_args.model_id:
    #     # # monkey patch the vision model
    #     # replace_qwen2_5_vision() # TODO: 2替换这部分看一下是否影响整体，至少可以关掉vision这部分
    ...
    #     # if use_liger: # TODO: 2是一个优化库，我可以只保留除了vision部分，暂时不调整，有地方修改了整体的forward,等着vision部分修改后,根据forward的修改情况再调整。
    #     #               # 暂时就全部关掉。至少可以开启llm部分的
    ...
    # TODO: 2lora部分可以只开llm部分的，暂时都不开启了。默认关闭的状态
    ...
    if training_args.bits in [4,8]: # 配置文件中这个地方是16，就暂时不修改这里，也暂时不清楚这里是否影响vision部分的改动
    ...
    # TODO: 这里需要做修改，应该是不会再有merger这个模块了
    ...
    if training_args.lora_enable:
        ...
        if not training_args.freeze_merger:
            for name, param in model.named_parameters():
                if "to_visual_latent" in name:
                    param.requires_grad = True
    # TODO: processor部分应该也会有变化，看情况做调整
    ...
    #         sampler=data_module["train_sampler"],
    #         collate_fn=data_module["data_collator"],
    #         drop_last=False,
    #         num_workers=training_args.dataloader_num_workers,
    #         pin_memory=training_args.dataloader_pin_memory,
    #         prefetch_factor=training_args.dataloader_prefetch_factor if hasattr(training_args, "dataloader_prefetch_factor") else None,
    #         persistent_workers=training_args.dataloader_persistent_workers if hasattr(training_args, "dataloader_persistent_workers") else False,
    #     )
    
    # # 替换get_train_dataloader方法
    # trainer.get_train_dataloader = custom_get_train_dataloader

    ## ======================== 改动3: 启用自定义 DataLoader ========================
    ## 原: 此段代码被注释掉，train_sampler 被创建但从未传入 Trainer，导致 Sampler 未生效
    ## 新: 取消注释并启用，通过覆盖 trainer.get_train_dataloader 将 ModalityAwareSampler
    ##     传入 DataLoader，使每个 batch 内模态一致
    ## ===========================================================================
    def custom_get_train_dataloader():
        train_dataset = trainer.train_dataset
        sampler = data_module["train_sampler"]
        sampler.set_epoch(int(trainer.state.epoch) if trainer.state.epoch else 0)
        return DataLoader(
            train_dataset,
            batch_size=training_args.per_device_train_batch_size,
            sampler=sampler,
