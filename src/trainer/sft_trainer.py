...
class QwenSFTTrainer(Trainer):
    ...
    def create_optimizer(self):
        # TODO: 初步来看，只需要修改这里的visual部分即可。甚至不需要修改更多地方
        """
        Setup the optimizer.
        ...
        if self.optimizer is None:
            ...
            if self.args.vision_lr is not None:
                lr_mapper["visual"] = self.args.vision_lr
                visual_parameters = [name for name, _ in opt_model.named_parameters() if "visual" in name and "to_visual_latent" not in name]
            # FIXME: 修正掉merger的部分
            if self.args.merger_lr is not None:
                lr_mapper["merger"] = self.args.merger_lr
                merger_parameters = [name for name, _ in opt_model.named_parameters() if "to_visual_latent" in name]
