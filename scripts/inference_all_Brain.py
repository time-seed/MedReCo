    ...
    with torch.no_grad():
        for batch_idx in tqdm(range(num_batches), desc="Processing batches"):
            ...
            for item in batch_data:
                try:
                    # Load image
                    img_tensor = load_BrainMRI_image_to_tensor(item['img_path'])
                    ...
                    batch_images.append(torch.zeros(1, 30, 480, 480))
                    batch_texts.append("")
            ...
            zero_vector = torch.ones(current_batch_size, dtype=torch.long, device=device) * 2
...
if __name__ == "__main__":
    # Configuration
    JSONL_PATH = "/mnt/petrelfs/zhangtengfei/public_dataset/RaTEScore/Brain/all/test.jsonl"  # TODO: 修改为你的jsonl文件路径
    OUTPUT_IMG_EMBEDS = "/mnt/petrelfs/zhangtengfei/RadIR/baseline/stage1/Brain_img_embeds.pt"  # 输出图像embeddings的路径
    OUTPUT_TEXT_EMBEDS = "/mnt/petrelfs/zhangtengfei/RadIR/baseline/stage1/Brain_text_embeds.pt"  # 输出文本embeddings的路径
    MODEL_PATH = "/mnt/petrelfs/zhangtengfei/RadIR/CT-CLIP/log/CXR_CT_Uni_CXRCT_1028_moe_expert_train_/CTClip.1600.pt"  # 模型路径
    BATCH_SIZE = 12  # TODO: 根据你的GPU内存调整batch size
