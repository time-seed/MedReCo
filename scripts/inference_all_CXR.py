...
def load_2d_image_to_tensor(image_path,resize=True):
            ...
            # img_array = (np.array(image) - mean) / (std + 1e-8)
    ...
    with torch.no_grad():
        for batch_idx in tqdm(range(num_batches), desc="Processing batches"):
            ...
            for item in batch_data:
                try:
                    # Load image
                    img_tensor = load_2d_image_to_tensor(item['img_path'])
                    ...
                    batch_images.append(torch.zeros(1, 1, 480, 480))
                    batch_texts.append("")
            ...
            zero_vector = torch.ones(current_batch_size, dtype=torch.long, device=device)
            text_latents, image_latents, _ = clip(
                text_tokens,
                image_batch,
                return_latents=True,
                device=device,
...
if __name__ == "__main__":
    # Configuration
    JSONL_PATH = "/mnt/petrelfs/zhangtengfei/RadIR/dataset/CXR/MIMIC_CXR/all.jsonl"  # TODO: 修改为你的jsonl文件路径
    # JSONL_PATH  = "/mnt/petrelfs/zhangtengfei/RadIR/baseline/medclip/extend_test.jsonl"
    # OUTPUT_IMG_EMBEDS = "/mnt/petrelfs/zhangtengfei/RadIR/baseline/stage1/CXR_img_embeds_2400_.pt"  # 输出图像embeddings的路径
    # OUTPUT_TEXT_EMBEDS = "/mnt/petrelfs/zhangtengfei/RadIR/baseline/stage1/CXR_text_embeds_2400_.pt"  # 输出文本embeddings的路径
    OUTPUT_IMG_EMBEDS = "/mnt/petrelfs/zhangtengfei/RadIR/baseline/stage1/CXR_img_embeds_2400_all.pt"  # 输出图像embeddings的路径
    OUTPUT_TEXT_EMBEDS = "/mnt/petrelfs/zhangtengfei/RadIR/baseline/stage1/CXR_text_embeds_2400_all.pt"  # 输出文本embeddings的路径
    MODEL_PATH = "/mnt/petrelfs/zhangtengfei/RadIR_log/CT-CLIP/new_log/1124_3modality_train_1e-4_with_text/CTClip.2400.pt"  # 模型路径
    BATCH_SIZE = 30  # TODO: 根据你的GPU内存调整batch size
